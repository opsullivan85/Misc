...
class Keyboard {
      ...
      static final BigDecimal numerator = new BigDecimal(0.1);
      ...
      public void getFitness(){
            ...
            for(int i = 0; i < WORD_LIST_LENGTH; i++){
                  ...
                  if(letter==10){
                        wordCount++;
                        this.fitness = this.fitness.add(fitnessFunction(wordFitness, wordCount, letterCount));/*this.fitness.add(wordFitness); f(wordCount)*wordFitness */
      ...
      public BigDecimal fitnessFunction(BigDecimal wordFitness, int wordCount, int letterCount){
            return wordFitness.multiply(numerator.divide(new BigDecimal(wordCount), RoundingMode.HALF_UP)).setScale(2, RoundingMode.HALF_UP); //wordFitness*0.1/wordCount  10.5-12
            //return wordFitness.pow(2).divide(new BigDecimal(100*wordCount), 32, RoundingMode.HALF_UP).multiply(new BigDecimal(letterCount)).subtract(new BigDecimal(0.1)); //wordFitness^2(*0.1/wordCount)/letterCount  10.5-12
            //return wordFitness.pow(2).multiply(new BigDecimal(letterCount/(100.*wordCount))).subtract(new BigDecimal(0.01));
            //return wordFitness.multiply(new BigDecimal((WORD_LIST_LENGTH-(double)wordCount)/WORD_LIST_LENGTH));//wordFitness*(WLL-wordCount)/WLL 153000-166000
            //return wordFitness; //170000-180000
            //return wordFitness.multiply(new BigDecimal(2500).divide(new BigDecimal(wordCount).add(new BigDecimal(1)), RoundingMode.HALF_UP).subtract(new BigDecimal(0.25)));//wordFitness*WLL/(4wordCount+1)-0.25
